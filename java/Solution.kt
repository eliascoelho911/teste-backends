...
class Solution {
  ...
  companion object {
    @JvmStatic
    fun processMessages(messages: List<String>): String {
      val eventos = Eventos()

      messages.map { eventoString ->
        eventos.criaEvento(eventoString.split(","))
      }

      eventos.executaTodosOsEventosValidos()

      return eventos.propostas.propostasValidas().joinToString(separator = ",") { it.id }
    }
  }
}
